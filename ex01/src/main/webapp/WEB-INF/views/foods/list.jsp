		...
		$(function(){
			...
			// 이벤트 처리 - 리스트에서 상품을 클릭했을때 
		 	$(".dataRow").click(function(){
		 		// 자료수집 - goods_no
				let foods_no = $(this).find(".foods_no").data("foods_no");
				// alert("goods_no= " + goods_no);
				// 여기선 ""안에 공백이 있어서는 안됨 있으면 그것도 다 불러옴
				location="view.do?foods_no=" + foods_no + "&${pageObject.pageQuery}" + "&${foodsSearchVO.searchQuery}";
			});
				...
				<div class="input-group mb-3">
					...
					<select class="form-control" id="cate_code1" name="cate_code1">
					 ...
					 <c:forEach items="${listBig }" var="vo">
					  	...
					  	<c:if test="${foodsSearchVO.cate_code1 == vo.cate_code1 }">
					  		selected
					  	</c:if>
					  	>${vo.cate_name }</option>
					 </c:forEach>
					 ...
					 <select class="form-control" id="cate_code2" name="cate_code2">
				  	...
				  	<c:forEach items="${listMid }" var="vo">
				  		...
				  		<c:if test="${foodsSearchVO.cate_code2 == vo.cate_code2 }">
					  		selected
					  	</c:if>
				  		>${vo.cate_name }</option>
				  	</c:forEach>
				  	 ...
				  	 <input type="text" class="form-control" placeholder="상품명 검색" id="foods_name" name="foods_name" 
				  	 value="${foodsSearchVO.foods_name }">
				  	 <input type="text" class="form-control" placeholder="최저가격입력" id="min_price" name="min_price" 
				  	 value="${foodsSearchVO.min_price }">
				  	 <input type="text" class="form-control" placeholder="최고가격입력" id="max_price" name="max_price" 
				  	 value="${foodsSearchVO.max_price }">
	 				 <div class="input-group-prepend">
		   		...
		   		<c:forEach items="${list }" var="vo" varStatus="vs">
		   			...
		   			<div class="col-md-3 dataRow">
				      	<div class="card" style="width:100%">
							  ...
							  <p class="card-text"> 
							  	<div class="foods_no" data-foods_no="${vo.foods_no }">
									상품번호 : ${vo.foods_no }
							  	</div>
							  	<div>
							 	 	상품명 : ${vo.foods_name } 
